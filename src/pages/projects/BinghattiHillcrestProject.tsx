...
import { 
  ...
  Building2, Sparkles, Dumbbell, Car, Waves, Users, TreePine, Baby, Shield
} from "lucide-react";
            ...
            <div className="container-luxury">
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <span className="badge-tag flex items-center gap-1"><Users className="w-3 h-3" />Family Living</span>
              ...
              <div className="lg:col-span-2 space-y-12">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {projectData.amenities.map((amenity, index) => (
                      <div key={index} className="flex items-start gap-4 p-4 bg-card border border-border rounded-xl">
                        <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center flex-shrink-0"><amenity.icon className="w-6 h-6 text-primary" /></div>
                        <div><h3 className="font-semibold text-foreground mb-1">{amenity.name}</h3><p className="text-sm text-muted-foreground">{amenity.description}</p></div>
                      </div>
                    ))}
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  ...
                  <div className="bg-card border border-border rounded-xl p-6">
                    <p className="text-muted-foreground mb-6">Hillcrest is ideally positioned in Arjan, part of the vibrant Dubailand area.</p>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-xl p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">Binghatti Hillcrest is ideal for growing families seeking affordable comfort in a community-focused environment. The Arjan area offers excellent value compared to central Dubai locations.</p>
                    </div>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="lg:col-span-1">
                <div className="sticky top-24 space-y-6">
                  ...
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="bg-card border border-border rounded-xl p-6">
                    ...
                    <div className="space-y-3">
                      <Button variant="outline" className="w-full justify-start" onClick={() => openLeadCapture({ projectName: projectData.name, ctaType: "Download Floor Plan" })}><Download className="w-4 h-4 mr-2" />Download Plan</Button>
                      <Button variant="outline" className="w-full justify-start" onClick={() => openLeadCapture({ projectName: projectData.name, ctaType: "Get Pricing" })}><Download className="w-4 h-4 mr-2" />Get Pricing</Button>
                    </div>
                  </motion.div>
                </div>
              </div>
